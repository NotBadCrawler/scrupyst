...
class AsyncioSleepSpiderMiddleware2:
    async def process_start(self, start):
        await maybe_deferred_to_future(asyncio_sleep(SLEEP_SECONDS))
        async for item_or_request in start:
            yield item_or_request
...
class TestMain:
    async def _test(self, spider_middlewares, spider_cls, expected_items):
        ...
        settings = {
            ...
            "TELNETCONSOLE_ENABLED": False,  # Disable deprecated telnet extension
        }
    ...
    @pytest.mark.asyncio
    async def test_asyncio_sleep_single_v2(self):
        await self._test_sleep([AsyncioSleepSpiderMiddleware2])

    @pytest.mark.asyncio
    async def test_asyncio_sleep_multiple_v2(self):
        await self._test_sleep(
            [NoOpSpiderMiddleware, AsyncioSleepSpiderMiddleware2, NoOpSpiderMiddleware]
        )
