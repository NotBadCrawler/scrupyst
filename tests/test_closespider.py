...
class TestCloseSpider:
    ...
    @pytest.mark.asyncio
    async def test_closespider_itemcount(self):
        close_on = 5
    ...
    @pytest.mark.asyncio
    async def test_closespider_pagecount(self):
        close_on = 5
    ...
    @pytest.mark.asyncio
    async def test_closespider_pagecount_no_item(self):
        close_on = 5
        max_items = 5
    ...
    @pytest.mark.asyncio
    async def test_closespider_pagecount_no_item_with_pagecount(self):
    ...
    @pytest.mark.asyncio
    async def test_closespider_timeout_no_item(self):
        timeout = 1
        crawler = get_crawler(SlowSpider, {"CLOSESPIDER_TIMEOUT_NO_ITEM": timeout})
        await crawler.crawl(n=3, mockserver=self.mockserver)
        reason = crawler.spider.meta["close_reason"]
