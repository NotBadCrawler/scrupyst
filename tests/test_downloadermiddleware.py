...
class TestManagerBase:
    ...
    # should be a fixture but async fixtures that use Futures are problematic with some test frameworks
    @asynccontextmanager
    ...
    async def _download(
        ...
        def download_func(request: Request) -> asyncio.Future[Response]:
...
class TestMiddlewareUsingFutures(TestManagerBase):
    """Middlewares using Futures should work"""
    ...
    async def test_future(self):
        ...
        async with self.get_mwman() as mwman:
            mwman._add_middleware(FutureMiddleware())
            result = await maybe_deferred_to_future(mwman.download(download_func, req))
        assert result is resp
        assert not download_func.called
...
class TestDownloadDeprecated(TestManagerBase):
    ...
    async def test_download_func_spider_arg(self):
        ...
        def download_func(request: Request, spider: Spider) -> asyncio.Future[Response]:
            future = asyncio.Future()
            future.set_result(resp)
            return future
